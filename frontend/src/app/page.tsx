        ...
        <main className="flex-1 md:ml-72 main-scroll">
          ...
          <div className="p-4 max-w-7xl mx-auto">
            <header className="flex items-center justify-between p-2 border-b border-gray-800 bg-black sticky top-0 z-20">
              <div className="flex items-center gap-2"><button aria-label="Open menu" onClick={() => setMobileDrawerOpen(true)} className="md:hidden px-2 py-1 text-xs rounded-sm panel-transparent">☰</button><div className="text-4xl">Noted</div></div>
                  ...
                  {filtered.map(note => (
                    <SortableItem id={note._id} key={note._id}>
                      <div onClick={() => openEdit(note)}>
                        <NoteCard note={note} onClick={() => openEdit(note)} onDelete={() => deleteNote(note._id)} />
                      </div>
                    </SortableItem>
                  ))}
                </section>
